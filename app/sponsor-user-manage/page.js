...
import UsersTable from "../components/usersTable/sponsor";
...
export default function sponsorUserManage() {
    ...
    const [companyFilter, setCompanyFilter] = useState(''); // Holds the company to filter by using the current user's company
    ...
    const [newUser, setNewUser] = useState({
      ...
      useEffect(() => {
        // Fetch user data and the current user's company on mount
        fetchUserData(page);
        fetchCurrentUserCompany();
      }, []); // The empty array ensures these effects run only once on mount
    
      const fetchCurrentUserCompany = async () => {
        try {
          const attributes = await fetchUserAttributes();
          const company = attributes['custom:company']; // Assuming 'fetchUserAttributes()' returns an object with the user's attributes
          setCompanyFilter(company); // Set the company filter to the current user's company
          setNewUser(prevState => ({
            ...prevState,
            company: company // Default the new user's company to the current user's company
          }));
        ...
        <ThemeProvider theme={darkTheme}>
            <Container>
                ...
                <UsersTable 
                    ...
                    userData={filteredData.filter(user => {
                        const userCompany = user.Attributes.find(attr => attr.Name === "custom:company")?.Value;
                        return companyFilter === userCompany;
                    })} 
